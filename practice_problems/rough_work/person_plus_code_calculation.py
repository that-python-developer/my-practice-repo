...
if __name__ == '__main__':
    ...
    start_time = datetime.now()  # start process logs
    python_process_name = 'person_plus_code_calculation'
    ...
    schema_name = 'edw_tab'
    table_name = 'dw_dim_person'
    stg_table_name = 'stg_person_lat_long_to_plus_code'

    query = f"""
        select 
            person_code, 
            person_location_latitude as latitude,
            person_location_longitude as longitude
        from {schema_name}.{table_name}
        where (
            person_location_latitude is not null
            or person_location_longitude is not null
        )
    """
    result_df = read_sql(query, conn)
    result_df['person_geo_plus_code'] = None

    for i, row in result_df.iterrows():
        try:
            person_id = row['person_code']
            latitude = row['latitude']
            longitude = row['longitude']
            row['person_geo_plus_code'] = encode(float(latitude), float(longitude))
        except Exception as e:
            continue

    result_df.to_sql(
        f'{stg_table_name}', con=conn,
        schema=f'{schema_name}', index=False, if_exists='replace'
    )
    # encode(19.218330, 72.978088)
    conn.close()
